...
cli::cli_alert_danger(
  "I wanted my last commit, that created bla, to be on a new branch!"
)
cli::cli_alert_info(
  "See {.url https://ohshitgit.com/#accidental-commit-master}"
)
...
tip <- function() {
  cli::cli_li(
    items = c(
      ...
      "If you are no longer on the main branch, {.code git switch main} or {.code git checkout main}",
      "{.code git reset HEAD~ --hard}",
      "{.code git switch feat-bla} or {.code git checkout feat-bla}",

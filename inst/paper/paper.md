...
![Diagram showing how saperlipopette is used. The saperlipopette R package contains functions such as `exo_time_machine()` and `exo_undo_commit()`. A call to a function like `exo_time_machine()` creates a temporary folder with a ready-made Git situation, from which one launches a new R session. In that folder, the user reads the instructions in R, and works on the Git challenge using the tools of their choice. A new call to the `exo_time_machine()` function would create yet another folder with the exact same exercise, making it easy to restart and repeat as needed.][/home/maelle/Documents/ropensci/PAPERS/jose/jose.png]{fig-alt=""}
...
# Lastly, from a trainer’s perspective, saperlipopette’s ready-made exercises can save time in demonstrating Git workflows.Pedagogical content of the software
